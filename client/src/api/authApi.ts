...
function extractErrorInfo(error: unknown): { message: string; status?: number; data?: AuthApiErrorData } {
...
export async function register(payload: {
  ...
  try {
    const response = await axiosClient.post<AuthResponse>(REGISTER_PATH, payload);
    return response;
  } catch (error: unknown) {
